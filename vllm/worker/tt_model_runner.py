...
class TTModelRunner(ModelRunnerBase[TTModelInput]):
    ...
    def prepare_model_input(
        ...
        if TTPlatform.always_compat_sampling:
            ...
            for seq_group_metadata in seq_group_metadata_list:
                ...
                for key in SAMPLING_PARAM_KEYS:
                    ...
                    elif current_value != param_values[key]:
        ...
        if is_prompt:
            ...
            seq_ids_tensor = torch.tensor(seq_groups_list,
                                          dtype=torch.int32,
                                          device="cpu")
    ...
    def _execute_model_single_step(self,
        ...
        if model_input.output_tokens is not None:
            execute_model_kwargs["output_tokens"] = model_input.output_tokens
        ...
        if not is_decode:
            ...
            if self.dp_kv_cache:
                ...
                if execute_model_kwargs.get("output_tokens") is not None:
                    execute_model_kwargs[
                        "output_tokens"] = execute_model_kwargs[
                            "output_tokens"][inverse_perm_indices]

                # permute the sampling_params if present
                # (already padded to max_num_seqs)
                if ("sampling_params" in execute_model_kwargs and
                        execute_model_kwargs["sampling_params"] is not None):
                    tt_sampling_params = execute_model_kwargs[
                        "sampling_params"]
                    # Permute each sampling parameter list using the
                    # full inverse_perm_indices
                    if isinstance(tt_sampling_params.temperature,
                        ...
                        assert all(
                            ...
                            f"{inverse_perm_indices.tolist()} for sampling "
                            f"param list of length {max_idx}")
                        permuted_temp = [
